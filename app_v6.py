...
default_csv_path = 'securityserver2.csv'

# Load the full data using pandas for analysis and graphing
df_full = pd.read_csv(default_csv_path)

# Create two columns for layout with expanded width
col1, col2 = st.columns([5, 5])  # Increase the ratio for more space
...
with col2:
    ...
    with st.expander("Open Chatbot Interface", expanded=True):
        # Load only 10 rows for the chatbot
        df_chatbot = pd.read_csv(default_csv_path, nrows=10)
